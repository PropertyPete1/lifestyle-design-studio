...
async function main() {
  ...
  if (!TEST_DELIVERY_ONLY && hasGeneratedToday(history, WINDOW)) {
    ...
    //
    // On a manual re-run a skip is expected. On the cron it is not — the schedule
    // meets each window once — so that one is promoted to the inbox.
    const scheduled = process.env.GITHUB_EVENT_NAME === "schedule";
    const reason =
      `A variant already exists for ${TODAY} ${WINDOW} — nothing generated. ` +
      (scheduled
        ? `This ran on the cron, which should meet each window exactly once. Either a run was ` +
          `replayed, or the window/date logic is selecting a window that is already filled.`
