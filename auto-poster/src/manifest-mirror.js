...
const ABOUT = [
  ...
  "A flagship post belongs to exactly one video. When two videos matched the same post — which happens because the caption template repeats for weeks — the one whose hand-post confirmation is inside 6 hours of the post keeps it, and a `contest` block records that and names the others. If two are that close, or neither is, every claimant is refused instead. Treat confidence 'medium' as what it says: enough to act on in aggregate, not enough to attribute a single number to a single video without checking.",
